...
_— https://www.youtube.com/watch?v=gAjR4_CbPpQ[Daft Punk]_

Our https://github.com/quarkusio/spring-quarkus-perf-comparison[Quarkus vs Spring CRUD benchmark] shows Quarkus delivering roughly 2x Spring's throughput on a REST/CRUD workload backed by PostgreSQL. The latest https://quarkus.io/benchmarks/[public results] confirm this.
...
Quarkus reaches ~14,700 req/s within 60 seconds. Spring reaches ~3,300 req/s by the same point and slowly climbs to ~7,800 by the end of the 2-minute warmup.

The shape matters. Quarkus's curve has a steep initial ramp and plateaus early. Spring's curve is still rising when the warmup ends — the application has not yet reached peak performance within the 2-minute window.
...
Both are starved during warmup. The difference is recovery: Quarkus needs fewer compilations to complete (shorter startup, less framework code) and its threads yield CPU more often, so C2 finishes before the load test.
...
https://openjdk.org/projects/leyden/[Project Leyden] in JDK 25 caches class loading, linking, and method profiling data in an AOT cache (https://openjdk.org/jeps/483[JEP 483], https://openjdk.org/jeps/514[JEP 514], https://openjdk.org/jeps/515[JEP 515]). However, **AOT Code Compilation** — caching the actual C2-compiled native code — is https://openjdk.org/jeps/8335368[not yet in JDK 25].

Without cached compiled code, **C2 still needs to compile hot methods at runtime**. In JDK 25, C2 compilations from the AOT cache's method profiles start concurrently with the application threads at startup.

image::warmup-tps-spring-leyden.png[Spring: jvm vs Leyden]

**spring4-leyden drops to 2,921 TPS — less than half of spring4-jvm (7,850).** During warmup, the two curves are close for the first 30 seconds, then spring4-jvm pulls ahead while spring4-leyden stalls at ~2,500-3,000.

image::warmup-tps-quarkus-leyden.png[Quarkus: jvm vs Leyden]

**quarkus3-leyden takes ~90 seconds to reach peak, vs ~60 seconds without Leyden.** The final throughput is also lower (13,060 vs 14,710).

Adding virtual threads recovers the lost throughput: spring4-virtual-leyden delivers 9,264 TPS, comparable to spring4-virtual (9,924).
...
spring4-leyden is the most severely affected configuration. To confirm that C2 starvation is the cause — not something inherent to the application or Leyden — we ran it with `-Xbatch`. This JVM flag forces the thread that triggers a compilation to block until the compilation completes. It is not a production-ready fix, but it answers a specific question: what happens if the compiler gets CPU time?
...
**Throughput jumps from 2,921 to ~7,900 TPS.** The compiled code reaches 33.8 MB (vs 20.8 MB without `-Xbatch`). The throughput loss comes from the compiler not getting enough CPU.

[NOTE]
====
**Troubleshooting hints:** To diagnose C2 starvation in your own application, enable https://docs.oracle.com/en/java/javase/25/jfapi/flight-recorder-api-programmers-guide.html[JDK Flight Recorder] with `-XX:StartFlightRecording=settings=profile,dumponexit=true` and look at:
...
* **https://docs.oracle.com/en/java/javase/25/jfapi/flight-recorder-api-programmers-guide.html[JDK Flight Recorder]** (`-XX:StartFlightRecording=settings=profile,dumponexit=true`) — `jdk.CompilerStatistics` for compiled code size over time, `jdk.Compilation` for individual compilation durations
...
The benchmark, methodology, and data are public: see https://github.com/quarkusio/spring-quarkus-perf-comparison/issues/591[issue #591] and https://github.com/quarkusio/spring-quarkus-perf-comparison/issues/420[issue #420].
