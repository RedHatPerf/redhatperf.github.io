...
Interestingly, JFR's `jdk.CompilerQueueUtilization` shows that Leyden actually **reduces** the total C2 work: spring4-leyden requests 2,622 C2 compilations with a queue peak of 459, while spring4-jvm requests 5,273 with a peak of 3,119. Despite less total work, Leyden performs worse. The faster startup puts the application under full load sooner, before the compiler has had time to make progress. Why exactly Leyden's AOT profiles interact poorly with the warmup phase remains an open question for a follow-up investigation.
