...
The impact on code quality is visible in JFR execution samples. Each sample's leaf frame is tagged as interpreted, JIT compiled (C1), or inlined (typically C2). For spring4-jvm during warmup, C1-compiled code dominates at **55-65%** of leaf frames, while C2-inlined code stays at just **30-40%**. The C2 queue (from `jdk.CompilerQueueUtilization`) peaks at **3,119 pending methods** and barely drains during warmup: `removedRate` drops to **zero** for 60+ seconds while the queue stays above 3,000. After the 30-second cooldown gives C2 the CPU it needs, the picture flips: C2-inlined code jumps to **70-84%** and C1 drops to ~20%. **The throughput jump after cooldown is the C2-compiled code finally running.**
