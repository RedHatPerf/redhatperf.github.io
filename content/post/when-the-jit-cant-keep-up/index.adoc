...
While investigating these results, we shared our findings with the OpenJDK community. https://github.com/ashu-mehra[Ashutosh Mehra] from the IBM OpenJDK team looked into the compilation policy and identified a https://github.com/openjdk/jdk25u/blob/master/src/hotspot/share/compiler/compilationPolicy.cpp#L1433[bug]: when training data is present, C2 is less aggressive about promoting hot methods to full optimization. Removing this guard in a patched build enables more C2 compilations, improves peak performance, and reduces time to peak for both frameworks. **The bug has not yet been officially filed; Leyden warmup numbers in JDK 25 should not be taken as representative.**
...
* **Leyden warmup in JDK 25 is affected by a https://github.com/openjdk/jdk25u/blob/master/src/hotspot/share/compiler/compilationPolicy.cpp#L1433[compilation policy bug].** Until this is fixed, and until AOT Code Compilation (https://openjdk.org/jeps/8335368[JEP draft 8335368]) ships, Leyden warmup numbers should not be taken as representative. Virtual threads can recover the lost throughput.
