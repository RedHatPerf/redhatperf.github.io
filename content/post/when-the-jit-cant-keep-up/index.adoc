...
Interestingly, JFR's `jdk.CompilerQueueUtilization` shows that Leyden actually **reduces** the total C2 work: spring4-leyden requests 2,622 C2 compilations with a queue peak of 459, while spring4-jvm requests 5,273 with a peak of 3,119. Despite less total work, Leyden performs worse. The training run for Spring uses `-Dspring.context.exit=onRefresh`, which only exercises startup and context initialization, not request handling. The AOT profiles therefore capture **startup methods**, and the C2 compilations they trigger are biased toward code that is not on the hot path during warmup. Without Leyden, live profiling drives C2 toward the methods that actually serve requests.
