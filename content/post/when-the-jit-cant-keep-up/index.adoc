...
**Leyden caveat:** enabling JDK Flight Recorder (`-XX:StartFlightRecording`) on a Leyden-enabled application can invalidate the AOT cache, because the `jdk.jfr` module added at runtime was not present during training (https://bugs.openjdk.org/browse/JDK-8386853[JDK-8386853]). For Quarkus, this also affects the two-step AOT cache creation process (https://github.com/quarkusio/quarkus/issues/54876[quarkus#54876]).
====
