...
The key is that this workload is **CPU-bound**: https://man7.org/linux/man-pages/man1/mpstat.1.html[mpstat] shows all 4 application cores at **95-97% usr with 0% idle** during warmup. The application threads are actively consuming all available CPU. It is not the mere presence of many threads that starves C2, it is that those threads are doing CPU-intensive work with no idle gaps for the compiler to run in.
