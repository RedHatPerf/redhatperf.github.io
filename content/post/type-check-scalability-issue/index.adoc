...
This article is a detailed and complementary content to https://youtu.be/G40VfIsnCdo[Quarkus Insights #107].
The mentioned Quarkus Insights episode treats a breed of the scary (was registered at Halloween on purpose) JDK issue https://bugs.openjdk.org/browse/JDK-8180450["JDK-8180450: secondary_super_cache does not scale well"]
that has caused massive performance degradation on several middleware frameworks eg Netty, Vert-x, Mutiny, Hibernate, Quarkus...

== What's JDK-8180450?

For who hasn't yet watched https://youtu.be/G40VfIsnCdo[Quarkus Insights #107], `JDK-8180450` is a severe scalability penalty happening while
...
among multiple threads, further causing https://en.wikipedia.org/wiki/False_sharing[False Sharing] with its surrounding fields eg `Klass::_secondary_supers`.
...
To make it more concrete: https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/lang/Integer.html[`Integer`] 's implement both `Comparable` and `Serializable`;
`_secondary_supers` contains both `Comparable` and `Serializable`, while `_secondary_super_cache` is used like this:
```java
...
```
The `checkcast` on `useComparable` doesn't need to search `Comparable` on the `_secondary_supers` array, but can
use `_secondary_super_cache`, saving useless work.

So, *How to cause such massive updates to the cache field*?

The answer is simpler then it seems: making it ping-pong between one or more values contained in `_secondary_supers`!
...
```
The comments on the code shows what happen if such code is called by a single thread, causing twice `_secondary_super_cache`
invalidations for each `compareAndSerializable` method execution, but

*What happen if there are more caller threads*?
...
https://www.techempower.com/[Techempower Benchmark] to validate https://quarkus.io[Quarkus]'s performance regressions and
...
The scariest part has yet to come: what we've discovered while investigating on the https://github.com/netty/netty/issues/12708[most severe case]
...
Furthermore, few weeks after the Quarkus Insights recording, Netflix has published an https://netflixtechblog.com/seeing-through-hardware-counters-a-journey-to-threefold-performance-increase-2721924a2822[article]
...
the latter provided some hint that something wrong was going on.

image::async_profiler_2_8_broken.png[Async Profiler 2.8 plaintext CPU flamegraph]

The above https://netflixtechblog.com/java-in-flames-e763b3d32166[flamegraph] shows at the bottom left of the graph
some costy method, but failing to report the right ones: not useful to fix it!
...
which modifies the x86 register used to track the https://en.wikipedia.org/wiki/Stack_register[Stack Pointer]
and key to detect the sampled trace caller method (more on this https://github.com/jvm-profiling-tools/async-profiler/issues/673#issuecomment-1304504745[Async Profiler issue's comment]).
...
has been https://github.com/openjdk/jdk11u-dev/pull/1524[back-ported to JDK 11] as well,
making this issue easier to be spotted by existing traditional (`AsyncGetCallTrace`-based) Java Profilers: a proper "Community Happy End".

==== ...and False Sharing?

In addition to the above issue, https://en.wikipedia.org/wiki/False_sharing[False Sharing] is responsible to make profiling tools
even more confused.

On any invalidation of `Klass::_secondary_super_cache`, a search over `Klass::_secondary_supers` is performed,
getting a severe penalty (if sitting in the same cache-line of the "noisy" field) and impacting other innocent code paths too:
...
```
The snippet above shows that performing a type check over a not implemented interface always
perform a slow path search over `_secondary_supers`, forcing loading the related field and causing
some ping-pong of cache coherency traffic to happen, slowing down the innocent load instruction and making a profiler
to highlight it.

The issue is better detailed in this https://youtu.be/G40VfIsnCdo?t=1411[Quarkus Insights part]
...
Thanks both, the JDK Optimizing Compiler can drop type check operations not making the issue to happen.
...
This smart compiler behaviour, together with the previous mentioned reasons, has made this issue to lie dormant and unnoticed for some time,
especially on benchmarking rigs, where the simplicity of the use case was leaving the compiler more
...
we've developed a https://github.com/raphw/byte-buddy[ByteBuddy]-powered https://github.com/RedHatPerf/type-pollution-agent[Java Agent] that has helped fixing the
...
the https://bugs.openjdk.org/browse/JDK-8180450[scalability JDK issue] has raised priority and targeting the Java 21 release.

=== Is it still worthy to fix our code?

As usual: *Depends(TM)*.

As said in the mentioned Quarkus Insights, some fixes have helped to use different and mode idiomatic code
solutions to replace a misuse (often abuse) of the existing type check-based logic, often leading to better performance and,
