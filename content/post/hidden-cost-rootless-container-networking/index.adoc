...
image::diff-flamegraph-gap.png[Differential flamegraph: perf-lab vs local, link=diff-flamegraph-gap.svg]
...
**With host networking, Quarkus throughput improves by 55% while Spring moves by +2.3% — a difference too small to draw conclusions from without multiple runs.** Disabling the firewall on top recovers another 8% for Quarkus, bringing the ratio from 1.19x back to 1.97x — close to the perf-lab's 2.08x.
...
image::diff-flamegraph.png[Differential flamegraph: default pasta vs host networking, link=diff-flamegraph.svg]
